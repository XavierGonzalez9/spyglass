...
class Spyglass:
    ...
    def _attach_live_keylogger_to_alert_engine(self, timeout_seconds: float = 3.0, poll_seconds: float = 0.1) -> bool:
        """Wait briefly for the live KeystrokeMonitor to exist, then subscribe AlertEngine to it."""
        if not self.alert_engine or not self.keylogger:
            return False

        deadline = time.time() + timeout_seconds
        while time.time() < deadline:
            live_monitor = getattr(self.keylogger, 'keylogger', None)
            if live_monitor is not None:
                self.alert_engine.subscribe_to_keylogger(live_monitor)
                logging.info("Alert engine subscribed to live keystroke monitor")
                return True
            time.sleep(poll_seconds)

        logging.warning("Alert engine could not attach to the live keystroke monitor within %.1f seconds", timeout_seconds)
        return False
    ...
    def start_all_monitoring(self) -> None:
        ...
        if self.alert_engine:
            # Always start the alert engine first so the keystroke subscription is ready.
        ...
        if self.config and self.config.is_keylogger_enabled() and self.keylogger:
            started = self.keylogger.start_keylogger()
            if started:
                self._attach_live_keylogger_to_alert_engine()
            else:
                logging.warning("Keylogger did not start successfully during HIGH monitoring startup")
